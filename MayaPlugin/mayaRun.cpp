		...
		if (result) {
			...
			if ((msg & MNodeMessage::kAttributeSet) || (msg & MNodeMessage::kOtherPlugSet)) {
				...
				//if(msg & MNodeMessage::kIncomingDirection)
					//MStreamUtils::stdOutStream() << "incoming dir <-------" << endl;

				//else
					//MStreamUtils::stdOutStream() << "outgoing dir ------->" << endl;
				...
				bool msgExists = false;
				for (int i = 0; i < materialInfoToSend.size(); i++) {
					if (materialInfoToSend[i].materialName == lambertShader.name() && materialInfoToSend[i].msgHeader.cmdType != CMDTYPE::NEW_MATERIAL) {
						msgExists = true;
...
void transformWorldMatrixChanged(MObject &transformNode, MDagMessage::MatrixModifiedFlags &modified, void *clientData) {
	...
	*/
	...
	if (result && !transformNode.hasFn(MFn::kCamera)) {
		...
		//MMatrix test = path.inclusiveMatrix(); 
		//MStreamUtils::stdOutStream() << "path " << path.fullPathName() << endl;
		...
		if (result) {
			...
			// fill out mTransfInfo to add to msg vector
		...
		// if not directly connected, check if mesh in hierarchy
		else {
			...
			for (int j = 0; j < hierarchy.length(); j++) {
				...
				for (int i = 0; i < prntChildCnt; i++) {
					...
					// if the child is of type kTransform and has children, add to chirarchy 
					...
					// if the child is of type mesh, add to hierarchy and break loop
...
void GetMeshInfo(MFnMesh &mesh) {
	...
	*/
	...
	int index = findMesh(mesh.name().asChar());
...
void meshAttributeChanged(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* clientData) {
	...
	// used for when moving a face after extrude is used
		...
		if (result) {
			...
			// check if mesh exists in scene
			int index = findMesh(meshNode.name().asChar());
			if (index >= 0) {
...
void meshWorldMatrixChanged(MObject &transformNode, MDagMessage::MatrixModifiedFlags &modified, void *clientData) {
	...
	if (msgToSend) {
		...
		//MsgHeader msgHeader = {}; 
		mMatrixInfo.msgHeader.msgSize   = totalMsgSize;
	...
	if (result) {
		...
		MStreamUtils::stdOutStream() << "transfNode " << transfNode.name() << endl;
		...
		MMatrix transfMat   = transfNode.transformationMatrix(); 

		MStreamUtils::stdOutStream() << "-------------------------------" << endl;
		MStreamUtils::stdOutStream() << "worldMatrix " << worldMatrix << endl;
		MStreamUtils::stdOutStream() << "-------------------------------" << endl;
		MStreamUtils::stdOutStream() << "localMatrix " << localMatrix << endl;
		MStreamUtils::stdOutStream() << "-------------------------------" << endl;
		MStreamUtils::stdOutStream() << "transfMat " << transfMat << endl;
		MStreamUtils::stdOutStream() << "-------------------------------" << endl;
		MStreamUtils::stdOutStream() << endl;

		int parents = transfNode.parentCount();

		for (int i = 0; i < parents; i++) {
			MFnDagNode prnt(transfNode.parent(i)); 
			MStreamUtils::stdOutStream() << prnt.name() << endl;
...
void activeCamera(const MString &panelName, void* cliendData) {
	...
	double FOV		   = cameraNode.verticalFieldOfView();
	...
	if (isOrtographic) {
		...
		FOV = ortoWidth.verticalFieldOfView();
	}
...
void nodeNameChanged(MObject& node, const MString& str, void* clientData) {
	...
	*/
	...
	if (node.hasFn(MFn::kMesh)) {
		...
		int meshIndex = findMesh(oldName.c_str());
		if (meshIndex >= 0) {
	...
	*/
	...
	for (int i = 0; i < meshInfoToSend.size(); i++) {
		...
		if (meshInfoToSend[i].msgHeader.cmdType == CMDTYPE::NEW_NODE || meshInfoToSend[i].msgHeader.cmdType == CMDTYPE::UPDATE_NODE) {
			...
			//MStreamUtils::stdOutStream() << "meshInfoToSend[i].msgHeader.cmdType " << meshInfoToSend[i].msgHeader.cmdType << " for " << meshInfoToSend[i].meshName << "\n";
